  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div class="btn-toolbar mb-3" role="toolbar" aria-label="Toolbar with button groups">
      <div class="btn-group mr-2" role="group" aria-label="First group">
        <button type="button" class="btn btn-info" onclick="window.location.href='<?php echo base_url('sispek/kontrak'); ?>'">
          <i class="fas fa-plus"></i> Buat SPK Baru
        </button>
        <button type="button" class="btn btn-info" onclick="window.location.href='<?php echo base_url('sispek/daftarspk'); ?>'">
          <i class="far fa-file-alt"></i> Daftar SPK
        </button>
        <button type="button" class="btn btn-info" onclick="window.location.href='<?php echo base_url('sispek/daftarkegiatan'); ?>'">
          <i class="fas fa-chart-pie"></i> Daftar Kegiatan
        </button>
        <button disabled type="button" class="btn btn-info" onclick="window.location.href='<?php echo base_url('sispek/daftarbast'); ?>'">
  ...
  <div class="card shadow mb-4">
    <div class="card-body">
      <div class="table-responsive">
        ...
        <form method="get" action="<?php echo base_url('sispek/daftarbast'); ?>" id="filterForm" class="form-inline mb-4">
          ...
          <tbody>
            <?php foreach ($daftarbast as $row) : ?>
              <tr style="text-align: center;">
                ...
                <td>
                  <!-- Tombol View -->
                  <a title="View" class="btn btn-primary btn-sm btn-view" data-no_spk="<?php echo $row->no_spk; ?>" href="<?php echo base_url('sispek/daftarspk/detail/') . substr($row->no_spk, 0, 4) . "-" . date('d-m-Y', strtotime($row->tanggal_spk)); ?>">
                    <i class="fa fa-eye"></i>
                  </a>

                  <!-- Tombol cetak -->
                  <a title="Cetak / Print" href="<?php echo base_url('sispek/daftarbast/cetak_bast/') . substr($row->no_spk, 0, 4) . "/" . date('d-m-Y', strtotime($row->tanggal_spk)); ?>" class="btn btn-info btn-sm" target="_blank">
